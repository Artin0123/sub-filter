...
function html(body: string, noCache = false): Response {
	...
	return new Response(`<!doctype html><meta charset="utf-8"/>${body}`, { headers });
}
...
async function requireLogin(req: Request, env: Env): Promise<boolean> {
	...
	const payload = await verifyCookie(env.ADMIN_PASSWORD, cookie);
	return !!payload;
}
...
async function handleAdminPage(request: Request, env: Env): Promise<Response> {
	...
	return html(`
		<style>
			body{font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:1000px;margin:40px auto;padding:0 16px}
		...
		document.getElementById('refresh').addEventListener('click', async ()=>{
			...
			s.textContent = r.ok
			  ? 'Updated: ' + (j.updated) + ', records=' + (j.records) + ', chunks=' + ((j && j.chunks && j.chunks.total) || 0)
			  : 'Failed';
...
async function handleRefresh(request: Request, env: Env): Promise<Response> {
	...
	const okCookie = await requireLogin(request, env);
	let ok = okCookie;
	if (!ok) {
